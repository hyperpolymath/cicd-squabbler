...
The placeholders this template shipped with are rendered for
`cicd-squabbler`. To customize further, adjust the base image or the package
list in `Containerfile` and the features in `devcontainer.json`. Run
`just deps` to verify the environment after first launch.
