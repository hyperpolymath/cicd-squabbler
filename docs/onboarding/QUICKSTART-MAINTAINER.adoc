...
// Replace rsr-template-repo, cicd-squabbler, {{DEPS}} with actuals
...
| `$PREFIX/share/cicd-squabbler/`
| Data files, assets

| `$PREFIX/share/doc/cicd-squabbler/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/cicd-squabbler/config.toml`

Fallback: `$HOME/.config/cicd-squabbler/config.toml`
...
Or via OPSM: `opsm update cicd-squabbler`
...
[source,bash]
----
just install --prefix=/opt/cicd-squabbler-instance1 --config=/etc/cicd-squabbler/instance1.toml
just install --prefix=/opt/cicd-squabbler-instance2 --config=/etc/cicd-squabbler/instance2.toml
----
